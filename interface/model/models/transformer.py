...
class PositionalEncoding(nn.Module):
    ...
    def forward(self, input_len):
        ...
        #print('-----1. input_pos-----', input_pos)
        #print('-----1. input_pos-----', input_pos.size())
        #print('----2. max_seq_len', self.max_seq_len)
...
class Encoder(nn.Module):
    ...
    def forward(self, diagnosis_codes, mask, seq_time_step, input_len):
        #print('---0---: ', diagnosis_codes.size()) [8, 5, 9]
        #diagnosis_codes = diagnosis_codes.permute(1, 0, 2)
        #seq_time_step = torch.Tensor(seq_time_step).cuda().unsqueeze(2) / 180
        #print('---3. time_feature---: ', seq_time_step)
        seq_time_step = torch.Tensor(seq_time_step).unsqueeze(2) / 240
        time_feature = 1 - self.tanh(torch.pow(self.selection_layer(seq_time_step), 2))
        #print('---1.1. time_feature---: ', time_feature.size()) #[8, 10, 64]
        time_feature = self.time_layer(time_feature)
        #print('---1.2. time_feature---: ', time_feature.size())[8, 10, 256]
        #print('---1---: mask', mask.size())
        mask = mask.permute(1, 0, 2)
        #print('---2---: mask', mask.size())
        ...
        return output
class EncoderNontime(nn.Module):
    def __init__(self,
                 max_seq_len,
                 num_layers=1,
                 model_dim=256,
                 ...
                 ffn_dim=1024,
                 dropout=0.0):
        super(EncoderNontime, self).__init__()

        self.encoder_layers = nn.ModuleList(
            [EncoderLayer(model_dim, num_heads, ffn_dim, dropout) for _ in
             range(num_layers)])
        self.pos_embedding = PositionalEncoding(model_dim, max_seq_len)
        self.value_embedding = torch.nn.Linear(1, 256)
        self.relu = nn.ReLU()
        self.tanh = nn.Tanh()

    def forward(self, input_len, batch_non_time):
        #print('batch_non_time: ',batch_non_time.unsqueeze(2).size())
        output = self.value_embedding(batch_non_time.unsqueeze(2))
        #print('-----0. input_len.unsqueeze(1)', input_len.unsqueeze(1))
        output_pos, ind_pos = self.pos_embedding(input_len)
        #print('-----3. output_pos', output_pos)
        #print('-----3. output_pos', output_pos.size())
        #print('-----3. output', output.size())
        output += output_pos
        self_attention_mask = padding_mask(ind_pos, ind_pos)
...
class EncoderNew(nn.Module):
    ...
    def forward(self, diagnosis_codes, mask, mask_code, seq_time_step, input_len):
        ...
        time_feature = 1 - self.tanh(torch.pow(self.selection_layer(seq_time_step), 2))
        time_feature = self.time_layer(time_feature)
        #print('----1----: ', mask_code.squeeze(3).size())
        #print('----2----: ', self.pre_embedding(diagnosis_codes).size())
        #mask_code = mask_code.squeeze(3)
        #mask_code = mask_code.permute(2, 1, 0)
        #print('----3----: ', self.bias_embedding.size())
        temp = (self.pre_embedding(diagnosis_codes) * mask_code.squeeze(3)).sum(dim=2) + self.bias_embedding
        #print('-----4-----: ', temp.size())
        output = (self.pre_embedding(diagnosis_codes) * mask_code.squeeze(3)).sum(dim=2) + self.bias_embedding
        output += time_feature
        output_pos, ind_pos = self.pos_embedding(input_len.unsqueeze(1))
        output += output_pos
        self_attention_mask = padding_mask(ind_pos, ind_pos)
...
class TransformerTime(nn.Module):
    def __init__(self, n_diagnosis_codes, batch_size, options):
        ...
        self.temperature_encoder = EncoderNontime(24, num_layers=options['layer'])
        self.temperature_layer = torch.nn.Linear(256, 6)
        self.nontime_layer = torch.nn.Linear(9, 24)
        ...
        self.predict_layer = torch.nn.Linear(54, 2) # 24+1
        self.softmax_layer = torch.nn.Softmax() # 24+1
        # dropout layer
    ...
    def forward(self, seq_dignosis_codes, seq_time_step, batch_labels, options, maxlen):
        ...
        diagnosis_codes, labels, mask, mask_final, mask_code = units.pad_matrix_new(seq_dignosis_codes, batch_labels, options)
        ...
        features = self.feature_encoder(diagnosis_codes, mask_mult, seq_time_step, lengths)
        ...
        #predictions = self.predict_layer(predictions)
        #predictions = self.softmax_layer(predictions)
        #predictions = predictions.unsqueeze(2)

        final_p = self.softmax_layer(predictions)
        labels = torch.LongTensor(labels)
        if options['use_gpu']:
            labels = labels.cuda()
        return final_p, labels, self_weight
