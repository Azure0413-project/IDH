...
// ==========================================
// 正式版：偵測到真實的緊急狀況時，自動彈出護理處置表單
// ==========================================
document.addEventListener("DOMContentLoaded", function() {
    // 尋找真實帶有 alert-trigger 的危險區塊
    let urgentAlert = document.querySelector('.alert-trigger');
    
    if (urgentAlert) {
        // 往上找到包含這個區塊的整張病床 (li 標籤)
        let bedElement = urgentAlert.closest('li.bed');
        
        if (bedElement) {
            console.log("🚨 偵測到未處理的緊急狀況，系統自動點擊病床彈出處置畫面...");
            bedElement.click();
        }
    }
});