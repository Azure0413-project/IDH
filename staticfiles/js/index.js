...
if (typeof rootUrl === 'undefined') {
    var rootUrl = window.location.origin + "/index/";
}
...
function clear() {
  ...
  if (area[4] == "get_record") {
    ...
    let targetArea = area[5];
    // 檢查 rootUrl 是否已經以 '/' 結尾，避免重複產生 '//'
    window.location.href = window.location.origin + "/index/" + targetArea;
    document.getElementsByClassName("modal-left")[0].style.display = "flex";
    document.getElementsByClassName("modal-right")[0].style.display = "flex";
    document.getElementsByClassName("modal-table")[0].classList.add("hidden");
    document.getElementById("left").src = "/static/img/left_inactive.svg";
...
function SwitchNurseList() {
  const targetUrl = rootUrl.endsWith('/') ? rootUrl + "dashboard" : rootUrl + "/dashboard";
...
document.addEventListener("DOMContentLoaded", function() {
  ...
  // 【修正點】檢查元素是否存在，避免 null 報錯
  if (!handleSelect || !handleTime) {
    console.warn("找不到處置時間選單元素，跳過初始化。");
    return; 
  }
